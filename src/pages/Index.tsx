    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-primary selection:text-primary-foreground">
      ...
      <nav className="sticky top-0 z-40 w-full bg-background/90 backdrop-blur-md border-b border-border px-4 md:px-12 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3 group cursor-pointer">
          ...
          <div className="w-9 h-9 rounded-full bg-secondary border border-border flex items-center justify-center group-hover:border-primary transition-colors duration-300">
            <User size={18} className="text-muted-foreground group-hover:text-primary transition-colors duration-300" />
          </div>
        </div>
      </nav>

      {/* HERO */}
      <header className="relative w-full flex flex-col items-start px-4 md:px-12 pt-10 pb-0 overflow-hidden">
        {/* Background */}
        ...
        <div className="relative z-20 w-full max-w-2xl">
          ...
          >
            <h2 className="text-4xl sm:text-5xl md:text-7xl font-black uppercase leading-[0.9] mb-3 tracking-tighter font-display text-balance">
              Welcome to the <span className="text-primary">Inner Circle</span>
            </h2>
            <p className="text-base md:text-xl text-muted-foreground mb-6 max-w-lg">
              Exclusive videos, guides, and health content designed to optimize your performance.
            </p>
            <div className="flex flex-wrap gap-3">
              <button className="px-6 py-3 bg-primary text-primary-foreground font-black uppercase tracking-wider rounded-sm hover:brightness-90 transition-all flex items-center gap-2 text-sm">
              ...
              <a
                ...
                className="px-6 py-3 bg-foreground/10 backdrop-blur-md border border-foreground/20 text-foreground font-black uppercase tracking-wider rounded-sm hover:bg-foreground/20 transition-all flex items-center gap-2 text-sm"
              >
        ...
        {/* SHIPPING FORM — right below the buttons */}
        <div className="relative z-20 w-full max-w-2xl pb-12">
          <ShippingForm />
        </div>
      </header>
          ...
          <div className="flex overflow-x-auto gap-4 px-4 md:px-12 pb-6 no-scrollbar">
            {LIFESTYLE.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex-none w-[280px] sm:w-[300px] p-6 rounded-xl bg-card border border-border relative overflow-hidden group"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            {BONUSES.map((bonus, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex items-center gap-5 p-6 md:p-8 rounded-xl bg-card border-2 border-primary/30 hover:border-primary transition-all duration-300 group"
                >
                  <div className="p-3 md:p-4 rounded-lg bg-primary/10 text-primary group-hover:scale-110 transition-transform duration-300 shrink-0">
                    <Icon size={28} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-xl font-black uppercase font-display leading-none mb-1">{bonus.title}</h3>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-primary hover:text-foreground transition-colors"
                    >
      ...
      {/* FOOTER */}
      <footer className="mt-16 border-t border-border bg-background px-4 md:px-12 py-12 md:py-16">
